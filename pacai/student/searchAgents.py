...
class CornersProblem(SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.satisfiedCorners = set()

        # raise NotImplementedError()

    def successorStates(self, state):
        # tuple rep: ((x,y), ({corners}))
        # print(state)
        ...
        for action in Directions.CARDINAL:
            ...
            if not hitsWall:
                ...
                newStateCorners = stateCorners.copy()

                for i in range(len(self.corners)):
                    corner = self.corners[i]
                    if corner == newPosition:
                        newStateCorners[i] = True

                successors.append(((newPosition, newStateCorners), action, 1))

        self._numExpanded += 1
        # print("successors: ", successors)
        # print(self.satisfiedCorners)

        return successors

        return super().successorStates(state)
    ...
    def isGoal(self, state):
        # satisfiedCorners = state[1]
        # if set(satisfiedCorners) == set(self.corners):
        #     return True
        # else:
        #     return False
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corners)):
        if not stateCorners[i]:
            manhattanDistances.append(manhattan(currentPosition, corners[i]))

    if not len(manhattanDistances):
        return 0

    # manhattanDistances.sort()

    return sum(manhattanDistances[:2])  # i guess the first 2 is the max it can be
...
def foodHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    # use starting gamestate
    # manhattan in for loop to get closest foodGrid
    # call maze distance on that

    # minDistance = float("inf")
    # minCords = (float("inf"), float("inf"))

    # for foodPosition in foodGrid.asList():
    #     if minDistance > manhattan(foodPosition, position):
    #         minCords = foodPosition
    #         minDistance = manhattan(foodPosition, position)

    # if minDistance == float("inf"):
    #     return 0
    # return maze(position, minCords, problem.startingGameState)

    distances = dict()

    for pos in foodGrid.asList():
        # distances[manhattan(pos, position)] = pos
        distances[maze(pos, position, problem.startingGameState)] = pos

    if not distances:
        return 0

    # lowestHeuristic = max(distances.keys())

    # final = maze(distances[lowestHeuristic], position, problem.startingGameState)

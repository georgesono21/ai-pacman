...
class PacmanGameState(AbstractGameState):
    ...
    def getGhostPosition(self, agentIndex):
        if agentIndex <= PACMAN_AGENT_INDEX or agentIndex >= self.getNumAgents():
            raise ValueError(
                "Invalid index passed to getGhostPosition(): %d." % (agentIndex)
            )
    ...
    def getGhostState(self, agentIndex):
        if agentIndex <= PACMAN_AGENT_INDEX or agentIndex >= self.getNumAgents():
            raise ValueError(
                "Invalid index passed to getGhostState(): %d." % (agentIndex)
            )
...
class PacmanRules:
    ...
    def getLegalActions(state):
        ...
        return Actions.getPossibleActions(
            agentState.getPosition(), agentState.getDirection(), state.getWalls()
        )
...
def readCommand(argv):
    ...
    noKeyboard = options.replay is None and (
        options.textGraphics or options.nullGraphics
    )
    ...
    args["ghosts"] = [
        BaseAgent.loadAgent(options.ghost, i + 1) for i in range(options.numGhosts)
    ]
...
def runGames(
    ...
    **kwargs
    ...
    if (numGames - numTraining) > 0:
        ...
        logging.info(
            "Record:        %s", ", ".join([["Loss", "Win"][int(w)] for w in wins])
        )
